	...
	if(mysqli_num_rows($_SESSION['result']) > 0){
				while ($row = mysqli_fetch_array($_SESSION['result']) ){
							...
							print "                  <td>".$row['checkout_date']."</td>\n";
											...
											print "                  <td>".$row['payment_status']."</td><td><a href=\"detail.php?booking=".$row['booking_id']."\"  \">Повече детайли</a></td><td><a class=\"delete\" href=\"deletebooking.php?booking=".$row['booking_id']."\"    >Изтрий</a></td></tr>";					
			
				}
			}
	else{
	print "<tr><td>Няма запис</td></tr>";
	}
